...
bool CFitProblem::calcSummaryFIM(const std::vector<CMatrix<double> *> &iMatVec, CMatrix<double> &oMat)
{
  if(!(iMatVec.size() > 0))
      return false;
...
bool CFitProblem::calcSummarySD(const std::vector<CVector<double> *> &iSDVec, CMatrix<double> &oMat)
{
  if(!(iSDVec.size() > 0))
    return false;
